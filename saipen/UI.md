# saipen UI РІР‚вЂќ Dark Golden Win95 (mandatory for all UI work)
...
`_ref\vintage\SKILL.md` РІР‚вЂќ read it only if it exists; never required.)
...
4. **Ultra-compact:** fits 640Р“вЂ”480. Padding 1-2px tight, 4px groups, 8px
...
Non-web UI (Qt, WinForms, TUI, C4D, AHK): map same tokens РІР‚вЂќ Verdana 9-12pt,
...
- Inputs always sunken; focus swaps highlight to `--textPrimary`; visible
  label mandatory РІР‚вЂќ placeholder is never the label.
- Tabs: active sunken, inactive raised, 0 gap between.
- Title bars 20px `--surface`; dialog bodies `--surfaceRaised`, РІвЂ°В¤2px padding.
...
reach; targets РІвЂ°Тђ24px primary / 16px secondary.

## QA before DONE

No rounded corner, no animation frame, Verdana renders non-aliased, fits
640Р“вЂ”480 without horizontal scroll, every hex traces to a token. LOG line:
